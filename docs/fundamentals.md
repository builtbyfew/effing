...
This walks you through building a video with [Effing](../README.md), step by step, in plain Node scripts using the library APIs directly. You'll start with a simple declarative composition, then generate custom images and animations programmatically.

We do everything from scratch here so you can fully understand the fundamentals. If you'd rather scaffold a project and learn through that workflow, read the [Tutorial](tutorial.md) instead — same destination, different route.

## Prerequisites

- Node.js 22+
- FFmpeg ships bundled with `@effing/ffs` (via `@effing/ffmpeg`) — no system installation needed.
...
[Watch the rendered video](https://static.effing.dev/fundamentals/1a-first.mp4)
...
[Watch the rendered video](https://static.effing.dev/fundamentals/1b-two-segments.mp4)
...
[Watch the rendered video](https://static.effing.dev/fundamentals/1c-effects-motion.mp4)
...
[Watch the rendered video](https://static.effing.dev/fundamentals/1d-audio.mp4)
...
[Watch the rendered video](https://static.effing.dev/fundamentals/2a-title-card.mp4)
...
[Watch the rendered video](https://static.effing.dev/fundamentals/2b-badge-stacked.mp4)
...
Three segments — a generated title card, an animated progress bar with bounce, and a panning photo — each with their own transition and audio. The complete code is in [`demos/fundamentals/3-final.tsx`](../demos/fundamentals/3-final.tsx).

[Watch the rendered video](https://static.effing.dev/fundamentals/3-final.mp4)
...
Here we used `data:` URLs to keep things self-contained, but things tend to click together nicely when you serve your annies and images as HTTP endpoints — each piece becomes a URL that composes with everything else. That's exactly what `@effing/create` sets up for you: run `npm create @effing my-effing-app` and you get a project where each annie is an HTTP route, each effie composes them by URL, and the whole thing can be previewed in the browser or rendered to video. The [Tutorial](tutorial.md) walks through that workflow end to end.
...
- **The [Tutorial](tutorial.md)** — scaffold a project with `@effing/create` and build with the framework
