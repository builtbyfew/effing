...
- 05a9531: Render with FFmpeg 9.0.1 (requires FFmpeg 8.0 or newer)
...
- a776c25: Reset the sample aspect ratio after cover-scaling a background clip
...
- ce3d907: Stop halving audio when global and segment tracks are mixed
...
- 3a362ef: Fix default H.264 CRF (28 → 23)
...
- 4af809a: Fix top-level audio fades being shifted by `seek`
...
- aeff511: Fix green flash and off-by-one when rendering animation layers with `delay`
...
- 91de6e2: Pad segment audio with silence so audio shorter than its segment doesn't cause the next segment's audio to start early
...
- 4212c57: Fix overlay enable window to respect `delay` on layers without explicit `from`
