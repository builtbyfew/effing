...
describe("S3CacheStorage", () => {
  ...
  describe("constructor", () => {
    test("creates S3Client with endpoint and credentials", async () => {
      const { S3Client } = await import("@aws-sdk/client-s3");
      const { S3CacheStorage } = await import("./cache");
  ...
  describe("getStream", () => {
    ...
    test("returns null when object does not exist (NoSuchKey)", async () => {
      ...
      const storage = new S3CacheStorage({ bucket: "my-bucket" });
  ...
  describe("TTL configuration", () => {
    test("uses default TTL of 60 minutes", async () => {
      const { Upload } = await import("@aws-sdk/lib-storage");
      const { S3CacheStorage } = await import("./cache");
